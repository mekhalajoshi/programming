 ...
 * 
 * Notes : 1) There can exactly be one queen per row. Otherwise the 2 queens in the row would
 ...
 * We can start placing a queen per row. When placing a queen on a row, col, we need to check if the position
 * is available based on what we have already placed. Then we move to the next
 * row.
 *
 */
public class NQueen {
