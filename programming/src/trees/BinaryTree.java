...
public class BinaryTree {
	...
	/**
	 * Returns the min root-to-leaf depth of the tree. Uses a recursive helper that
	 * recurs down to find the min depth.
	 */
	 ...
	 * root down to a leaf, such that adding up all the values along the path equals
	 * the given sum. Strategy: subtract the node value from the sum when recurring
	...
	public void postorderNoRecurssion(Node node) {
		if (node == null) {
			return;
		}
		Stack<Node> stack = new Stack<Node>();
	...
	public boolean isBalancedTree(Node node) {
		...
		if (Math.abs(lh - rh) <= 1 && isBalancedTree(node.left) && isBalancedTree(node.right))
			return true;
	 ...
	 * 
	 * BST_LCA LCA of 10 and 14 is 12 LCA of 14 and 8 is 8 LCA of 10 and 22 is 20
	 * Following is definition of LCA from Wikipedia: Let T be a rooted tree. The
	 * lowest common ancestor between two nodes n1 and n2 is defined as the lowest
	 * node in T that has both n1 and n2 as descendants (where we allow a node to be
	 ...
	 * twice the distance from the root to their lowest common ancestor. (Source
	 * Wiki)
	 */
	...
	public static void main(String[] args) {
		...
		System.out.println();
		System.out.print("Inorder No recurssion = ");
		tree.inorderNoRecursion(tree.root);
		System.out.println();
		System.out.print("Postorder = ");
		tree.postorder(tree.root);
		System.out.println();
		System.out.print("Postorder No Recursion = ");
		tree.postorderNoRecurssion(tree.root);
		System.out.println();
